...
## 1. Review Issues Summary

> Full 55-item checklist by theme: **[REVIEW_CHECKLIST.md](REVIEW_CHECKLIST.md)**

Synthesized from 4 ICML reviewers (vpXC, bPFr, PawB, iutV) and 3 AI model reviews (Claude, GPT-4o, Grok).

**Top priorities for acceptance** (must-fix before submission):

| # | Issue | Phase |
|---|-------|-------|
| 1 | **Reframe novelty** — de-emphasize the architecture, lead with (1) controlled benchmarking methodology and (2) dynamic sparsification. Explicitly contrast with iTransformer/LSINet. | E |
| 2 | **Add error bars to all tables** — point estimates with no CIs will be flagged immediately. Run 5–10 seeds per cell. | B/C |
| 3 | **Add DLinear/NLinear and iTransformer** — the most-cited "transformers not effective" baseline is absent. | B |
| 4 | **Add figures** — zero figures in a NeurIPS submission is a near-automatic reject. Need: perf-vs-ρ line plots, architecture diagram, attention heatmaps. | E |
| 5 | **Fix abstract overclaims** — Lasso wins on Lin/TS-Shift; soften to "on several dependency structures, particularly cross-sectional and nonlinear." | E |
| 6 | **Elevate M5 to main text** and explain the VAR/Ridge overfitting (train corr ≈ 1.0). | E |
| 7 | **Present TSCS-Shift fix as a finding**, not a silent fix. Add epoch-curve evidence and at least one architecture ablation (CTCT vs TCTC). | E |
| 8 | **Add Implementation Details** — optimizer, LR, batch size, stopping rule, hardware — for every model. | E |
| 9 | **Sparsity K sensitivity** — K=0.1 with no ablation will be criticized. Test K ∈ {0.01, 0.05, 0.1, 0.2, 0.5}. | C |
| 10 | **Update literature review** to 2024–2025 (iTransformer, PatchTST, DLinear, LSINet, etc.). | E |

**Already resolved** in the new notebook: TSCS-Shift failure (C1), val-set epoch selection (E1), GlobalLSTM (B1), TFT/TiDE/NBEATSx in main (B2), VAR baseline (B3), entmax15 (A4/F5), MSE+MAE on M5 (I2), VAR+LSTM on M5 (I1).
