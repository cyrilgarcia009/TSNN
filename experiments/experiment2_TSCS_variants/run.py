...
=====================================
Compares four TSCS_shift variants that differ on two axes:
  - TS lag  : fixed (=1) or random drawn from [1, 9]
  - CS shift: circular (fixed) or random derangement

Effect names in config.yaml / CSV:
  TSCS_FF  fixed lag=1,  circular CS
  TSCS_FR  fixed lag=1,  random derangement CS
  TSCS_RF  random lag,   circular CS
  TSCS_RR  random lag,   random derangement CS

Usage
-----
    # Full run
    python experiments/experiment2_TSCS_variants/run.py

    # Smoke test
    python experiments/experiment2_TSCS_variants/run.py \\
        --seeds 0 --rhos 0.1 0.2 --models TCTC Lasso
...
# ---------------------------------------------------------------------------
# TSCS variant definitions
# Keys are the effect names used throughout the CSV / config.
# max_ts_lag=1 → fixed lag of 1; max_ts_lag=9 → random lag in [1, 9].
# shuffle_cs=False → circular shift; shuffle_cs=True → random derangement.
# ---------------------------------------------------------------------------
TSCS_VARIANTS = {
    "TSCS_FF": {"max_ts_lag": 1, "shuffle_cs": False},
    "TSCS_FR": {"max_ts_lag": 1, "shuffle_cs": True},
    "TSCS_RF": {"max_ts_lag": 9, "shuffle_cs": False},
    "TSCS_RR": {"max_ts_lag": 9, "shuffle_cs": True},
}
...
def generate_dataset(effect, T, n_ts, n_f, rho, seed):
    """Generate (X, y, y_opt, generator) for a TSCS variant."""
    np.random.seed(seed)
    torch.manual_seed(seed)

    vcfg = TSCS_VARIANTS[effect]
    max_ts_lag = vcfg["max_ts_lag"]
    shuffle_cs = vcfg["shuffle_cs"]
    # random_ts_shift: randint(1, x) gives [1, x-1]; pass max_ts_lag+1 for [1,max_ts_lag].
    ...
    z.generate_dataset_gr_simple(
        ...
        list_type_effects=["TSCS_shift"] * n_f,
...
def _nf_rollout(nf, full_df, futr_exog_list, start_t, end_t):
    ...
    return np.stack(pred_steps, axis=0)
...
def run_theoc(effect, rho, generator_obj):
    # TheoC is undefined for TSCS effects
    val = np.nan
...
def parse_args():
    p = argparse.ArgumentParser(description="Run Experiment 2 — TSCS Variant Ablation")
    p.add_argument("--config", default=str(_HERE / "config.yaml"))
    p.add_argument("--effects", nargs="+", help="Override effects (subset of TSCS_FF/FR/RF/RR)")
...
def main():
    ...
    unknown = [e for e in effects if e not in TSCS_VARIANTS]
    if unknown:
        raise ValueError(f"Unknown effect(s): {unknown}. Must be one of {list(TSCS_VARIANTS)}")
    ...
    for seed in seeds:
        ...
        for effect in effects:
            for rho in rhos:
                ...
                try:
                    X, y, y_opt, gen_obj = generate_dataset(effect, T, n_ts, n_f, rho, seed)
